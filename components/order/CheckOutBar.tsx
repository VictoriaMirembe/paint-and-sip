...
export default function CheckOutBar({
  ...
  const [selectedTable, setSelectedTable] = useState<string>(
    tables.length > 0 ? tables[0].tableNumber : "Table 001"
  );
        ...
        <CardHeader className="flex flex-row items-start bg-muted/50 p-4">
          <div className="grid gap-0.3">
            <CardTitle className="group flex items-center gap-2 text-sm font-bold whitespace-nowrap">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  className="bg-slate-600 dark:bg-black text-white dark:text-white w-[130px] font-semibold rounded-sm text-sm px-4 py-2 cursor-pointer"
                  align="end"
                >
          ...
          <div className="ml-auto flex items-center gap-1">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                className="bg-slate-600 dark:bg-muted/50 text-white dark:text-white w-[80px] font-semibold rounded-sm text-sm px-4 py-2 cursor-pointer"
                align="end"
              >
                ...
                return (
                  dish && (
                    <li key={id} className="flex items-center justify-between">
                      ...
                      <span>ugx {(dish.amount * quantity).toFixed(2)}</span>
                    </li>
                  )
                );
              })}
          ...
          <div className="grid gap-3">
            ...
            <ScrollArea className="grid gap-3 mt-3 ">
              <div className="flex items-center gap-2">
                <Card
                  className="flex font-semibold text-xs items-center px-5  py-2  gap-1 text-black dark:text-white cursor-pointer border-[#5db8b9]"
                ...
                <Card
                  className="flex font-semibold text-xs items-center px-5 py-2  gap-1 text-black dark:text-white cursor-pointer hover:border-[#5db8b9]"
                ...
                <Card
                  className="flex font-semibold text-xs items-center px-5  py-2  gap-1 text-black dark:text-white hover:border-[#5db8b9] cursor-pointer"
          ...
          <div className="text-xs text-muted-foreground  w-full lg:pt-8 pb-2">
            <div className="items-center gap-2 flex w-full">
              ...
              {loading ? (
                ...
                >
                  <PiSpinner className="animate-spin" /> Processing{" "}
                </Button>
              ) : (
                <Button
                  className="w-full bg-[#1ba09d] dark:bg-white dark:hover:bg-black dark:hover:text-slate-100 flex gap-1 items-center"
                  size="sm"
                  onClick={() => makeOrder()}
                >
