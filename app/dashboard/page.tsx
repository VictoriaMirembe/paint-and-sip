...
import IngredientsStats from "@/components/IngredientsStats";
...
export default async function DashboardPage() {
  ...
  const tables = (await getTables()) || []; // Default to empty array if undefined
  ...
  const previousMonthKey = previousMonth.toLocaleString("default", {
    month: "short",
  });
  ...
  const trendingDishCurrentMonth = (orders || []).reduce(
    (acc: any, order: any) => {
      if (order.status === "served") {
  ...
  const trendingDishNameCurrentMonth = Object.keys(
    trendingDishCurrentMonth
  ).reduce(
    (a, b) =>
  ...
  const totalTrendingDishCurrentMonth =
    trendingDishCurrentMonth[trendingDishNameCurrentMonth] || 0;

  const previousMonthDate = new Date();
  previousMonthDate.setMonth(previousMonthDate.getMonth() - 1);
  const trendingDishPreviousMonth = (orders || []).reduce(
    (acc: any, order: any) => {
      const date = new Date(order.createdAt);
      const month = date.toLocaleString("default", { month: "short" });
      if (
        month ===
          previousMonthDate.toLocaleString("default", { month: "short" }) &&
        order.status === "served"
      ) {
        acc[order.dishName] = (acc[order.dishName] || 0) + 1;
      }
  ...
  const trendingDishNamePreviousMonth = Object.keys(
    trendingDishPreviousMonth
  ).reduce(
    (a, b) =>
  ...
  const totalTrendingDishPreviousMonth =
    trendingDishPreviousMonth[trendingDishNamePreviousMonth] || 0;

  const percentageChangeTrendingDish = totalTrendingDishPreviousMonth
    ? ((totalTrendingDishCurrentMonth - totalTrendingDishPreviousMonth) /
        totalTrendingDishPreviousMonth) *
      100
    : 0;

  const totalCurrentMonthSales = (orders || []).reduce(
    (acc: any, order: any) => {
      if (order.status === "served") {
        return acc + 1;
      }
  ...
  const percentageChangeTotalSales =
    ((totalCurrentMonthSales - previousMonthTotalSales) /
      previousMonthTotalSales) *
    100;

  const totalCurrentMonthOrders = (orders || []).filter(
    (order: any) => order.status === "served"
  ).length;
  const previousMonthTotalOrders = totalCurrentMonthOrders * 0.85;
  const percentageChangeTotalOrders =
    ((totalCurrentMonthOrders - previousMonthTotalOrders) /
        ...
        <div className="flex-1 space-y-4 md:p-6 p-2 lg:p-8 lg:pt-6 md:pt-6 pt-2">
          <Tabs defaultValue="overview" className="lg:space-y-4 ">
            <TabsList>
              ...
              <TabsTrigger value="analytics">Tables</TabsTrigger>
            </TabsList>
            <TabsContent value="overview" className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{`ugx ${totalCurrentMonth.toFixed(
                      2
                    )}`}</div>
                    <p className="text-xs text-muted-foreground">{`${percentageChange.toFixed(
                      1
                    )}% from last month`}</p>
                  </CardContent>
                </Card>
            ...
            <TabsContent value="ingredients" className="space-y-4">
              {/* <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4"> */}
              <IngredientsStats ingredients={ingredients} />
              {/* </div> */}
            </TabsContent>
